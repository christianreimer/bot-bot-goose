...
// and picking approved bots and decoys for a round.
//
...
//
// Every round has the same shape: 1 bot + 3 decoys. v1 selection is uniform-
// random over approved content; step 9 of the plan replaces the pickers with
// the slot E/P/B bandit (Thompson sampling) without changing this package's
// public API.
package puzzle
...
// decoy ids. Every round is 1 bot + 3 decoys.
//
// Returns ErrBadExplicitPick with a human-readable message on bad ids,
// duplicates, or count mismatches.
func ComposeRoundAnswersExplicit(ctx context.Context, d *db.DB, promptID uuid.UUID, botIDs, decoyIDs []uuid.UUID) ([]db.Answer, error) {
	const wantBots, wantDecoys = 1, 3
	if len(botIDs) != wantBots {
		return nil, &ErrBadExplicitPick{Msg: fmt.Sprintf("need %d bot id, got %d", wantBots, len(botIDs))}
	}
	if len(decoyIDs) != wantDecoys {
		return nil, &ErrBadExplicitPick{Msg: fmt.Sprintf("need %d decoy ids, got %d", wantDecoys, len(decoyIDs))}
	}
...
// ComposeRoundAnswers picks the 4 answers for one round: 1 bot + 3 decoys.
// Returns ErrInsufficientContent if the approved pool can't fill the round.
func ComposeRoundAnswers(ctx context.Context, d *db.DB, promptID uuid.UUID) ([]db.Answer, error) {
