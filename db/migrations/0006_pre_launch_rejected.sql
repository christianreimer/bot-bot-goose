...
-- the prelaunch reviewer: still-pending (ingested_decoy_id IS NULL) and
...
-- Once set, the row is excluded from the prelaunch deck's "under-supplied"
...
-- the prelaunch stats query.

ALTER TABLE pre_launch_submissions
    ADD COLUMN rejected_at TIMESTAMPTZ NULL;

-- The prelaunch deck counter only cares about live (non-rejected) rows; a
